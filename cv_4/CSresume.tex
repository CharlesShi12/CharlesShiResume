...
\vspace{-6.4mm}
\begin{itemize} \itemsep -2pt 
\item[$\circ$] Successfully managed hundreds of students and gained leadership experience after adapting to unforeseen circumstances. 
\vspace{1mm}
\item[$\circ$] Designed/planned final projects that assessed the students' problem solving skills while incorporating their interests.
\end{itemize}
\begin{small}
\section{PROJECTS}
\end{small} 
...
\vspace{-6.4mm}
\begin{itemize} \itemsep -2pt 
\item[$\circ$] Utilized Kotlin to design a server with unique IDs for each room and data structures that store each room's collaborators and their previous calculations.
\vspace{1mm}
\item[$\circ$] Developed a JavaScript client to communicate with the server, implemented WebSockets to handle the constant influx of data, and interacted with a RESTful API to compute equations inputted by the user. 
\end{itemize}
\vspace{-2.5mm}
\href{https://github.com/CharlesShi12/AI_Tumor_Scanner}{\textbf{AI Tumor Scanner}} — {\sl Python, TensorFlow/Keras, Streamlit, Neural Network}\hfill July 2020\vspace{-5mm}
\begin{itemize}[leftmargin=5.5mm]
...
\vspace{-6.4mm}
\begin{itemize} \itemsep -2pt 
\item[$\circ$] Built the neural network with TensorFlow/Keras and used data augmentation to train it with over \textbf{7,000} images.
\vspace{1mm}
\item[$\circ$] Tested and modified the neural network to obtain a final average accuracy of \textbf{95\%}. 
\end{itemize}
\vspace{-2.5mm}
...
\begin{itemize}[leftmargin=5.5mm]
\item Programmed an algorithm in Java that generates random English-like words using a model and highly optimized data structures. 
\end{itemize}
\vspace{-6.4mm}
\begin{itemize} \itemsep -2pt 
\item[$\circ$] Trained the model with over \textbf{80,000} English words to produce accurate/pronounceable outputs and designed a Trie data structure for efficiency.
\end{itemize}
\vspace{-2.5mm}
...
\vspace{-6.4mm}
\begin{itemize} \itemsep -2pt 
\item[$\circ$] Applied PPM raster image formatting and constructed a k-means clustering algorithm to find the dominant colors of an inputted image and filter/reduce the image based on those colors. 
\end{itemize}
\begin{small}
\section{SKILLS}
\end{small} 
