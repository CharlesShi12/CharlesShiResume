...
\usepackage{hyperref}
\resumewidth = 7.2in 
\geometry{left=.65in, top=.47in, right=.65in, bottom=.49in}
\newsectionwidth{0pt}  
\begin{document} 
\begin{footnotesize}
...
\item Created a similarity search algorithm called MedSearch that takes in the abstract of any COVID-19 research paper and returns other similar/related COVID-19 research papers to advance coronavirus research.
\vspace{1mm}
...
\vspace{-5mm}
\begin{itemize} \itemsep -2pt 
\item Develop a personalized patient dashboard that gives doctors and researchers an in-depth analysis of synthetic patient data through informative visualizations and statistics. 
\vspace{1mm}
\item Obtained Synthea-generated patient data and computed patient statistics by writing multiple GSQL queries in TigerGraph and programmed the dashboard and data visualizations using Dash and Plotly
\end{itemize}
\vspace{-2.5mm}
...
\end{small} 
\vspace{.5mm}
\href{https://github.com/CharlesShi12/CalcShare}{\textbf{Real-Time Collaborative Calculator}} \hfill August 2020\vspace{-5mm}
\begin{itemize} \itemsep -2pt 
\item Constructed an interactive web-based calculator that allows real-time collaboration between users. 
\vspace{1mm}
\item Utilized Kotlin to program a server with unique IDs for each room and data structures that store each room's collaborators and their previous calculations.
\vspace{1mm}
\item Developed a JavaScript client to communicate with the server, implemented WebSockets to effectively handle the constant influx of data, and interacted with a RESTful API to compute equations inputted by the user.  
\end{itemize}
\vspace{-2.5mm}
\href{https://github.com/CharlesShi12/AI_Tumor_Scanner}{\textbf{AI Tumor Scanner}} \hfill July 2020\vspace{-5mm}
\begin{itemize} \itemsep -2pt 
...
\end{itemize}
\vspace{-2.5mm}
\href{https://github.com/CharlesShi12/GibberishGenerator}{\textbf{Gibberish Generator}} \hfill May 2020\vspace{-5mm}
\begin{itemize} \itemsep -2pt 
\item Implemented an algorithm in Java that generates random English-like words using a model and optimized data structures.  
\vspace{-2.5mm}
...
\end{itemize}
\vspace{-3.25mm}
\href{https://github.com/CharlesShi12/ImageFilters}{\textbf{Image Filtering System}} \hfill March 2020\vspace{-5mm}
\begin{itemize} \itemsep -2pt
\item Programmed a Python program that uses unsupervised machine learning to filter/reduce an image down to however many core colors its users select.
\vspace{1mm}
...
\vspace{.5mm}
{\sl Languages:} 
Python, Java, HTML/CSS, JavaScript, Kotlin, GSQL, MATLAB \\
